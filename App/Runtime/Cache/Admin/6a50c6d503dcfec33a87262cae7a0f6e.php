...
<div style="float: left" id="my_menu" class="sdmenu">
    <?php if(is_array($authp)): $i = 0; $__LIST__ = $authp;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vop): $mod = ($i % 2 );++$i;?><div>
        <span><?php echo ($vop["auth_name"]); ?></span>
        <?php if(is_array($auths)): $i = 0; $__LIST__ = $auths;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vos): $mod = ($i % 2 );++$i; if($vos['auth_pid'] == $vop['auth_id']): ?><a href="/rbactest/index.php/Admin/<?php echo ($vos['auth_c']); ?>/<?php echo ($vos['auth_a']); ?>" target="mainFrame" onFocus="this.blur()"><?php echo ($vos["auth_name"]); ?></a><?php endif; endforeach; endif; else: echo "" ;endif; ?>
