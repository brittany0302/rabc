...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
    <tr>
        <td width="99%" align="left" valign="top">您的位置：系统设置&nbsp;&nbsp;>&nbsp;&nbsp;角色分配权限</td>
    </tr>
    <tr>
        <td align="left" valign="top" id="addinfo">
            <a href="<?php echo U('Auth/add');?>" target="mainFrame" onFocus="this.blur()" class="add">新增角色</a>
        </td>
    </tr>
    <tr>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
                    <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
                        ...
                        <td align="left" valign="middle" class="borderright borderbottom main-for">
                            <input type="text" name="role_name" value="<?php echo ($role_name); ?>" class="text-word">
                        </td>
                    </tr>
                    <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
                        <td align="right" valign="middle" class="borderright borderbottom bggray">选择权限：</td>
                        <td align="left" valign="middle" class="borderright borderbottom main-for">
                            <ul>
                                <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li>
                                    <input class="cb" id="<?php echo ($val["auth_id"]); ?>" pid="<?php echo ($val["auth_pid"]); ?>" type="checkbox" name="ch[]" value="<?php echo ($val["auth_id"]); ?>"
                                        <?php if(in_array($val['auth_id'],$row_auth_ids_arr)){ echo 'checked'; } ?>
                                    >
                                        <label for="<?php echo ($val["auth_id"]); ?>"><?php echo ($val["p"]); echo ($val["auth_name"]); ?></label>
                                    </li><?php endforeach; endif; else: echo "" ;endif; ?>
                                <!--<?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li>
                                        <?php if($val["auth_pid"] == 0): ?>{*if意味着根节点*}
                                            <?php if($i > 1): ?>{*if排除掉第一个根节点*}
                                                <hr color="red"><?php endif; ?>
                                        <?php else: ?>
                                            {*else意味着子节点*}
                                            <hr><?php endif; ?>
                                        <input type="checkbox">
                                        <?php echo ($val["p"]); echo ($val["auth_name"]); ?>
                                    </li><?php endforeach; endif; else: echo "" ;endif; ?>-->
                            </ul>
                    ...
                    <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
                        ...
                        <td align="left" valign="middle" class="borderright borderbottom main-for">
                            <input name="role_id" type="hidden" value="<?php echo ($_GET['role_id']); ?>">
...
<script>
    $('.cb').click(function () {
        ...
        if(all){
            ...
            $('.cb').each(function () {
                //选中父类,即当前id等于它的父id
                $('[id='+auth_pid+']').prop('checked','checked');
                //选中所有子类,即当前父id等于它的id
            ...
            $('.cb').each(function () {
                ...
                //再遍历同级有没有选中的,全都没被选中就不让父类选中,只要有一个同级选中就让父类选中
                if($('[pid='+auth_pid+']').is(':checked')){
                    bool = false;
                }
            });
            if(bool){
                //它和它的同级都没有被选中,让父类不被选中
                $('[id='+auth_pid+']').removeAttr('checked');
            }
        }
    })
</script>